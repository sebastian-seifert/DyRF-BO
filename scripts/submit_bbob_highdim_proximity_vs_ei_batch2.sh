...
echo "=================================================="
echo "Submitting BBOB High-D & Extreme EI Sweep - BATCH 2 (Tasks 5001 to 8640)"
echo "=================================================="

TASK_FILE="results/sweep_bbob_highdim_proximity_vs_ei/tasks.txt"

if [ ! -f "$TASK_FILE" ] || [ ! -s "$TASK_FILE" ]; then
    echo "Generating task file..."
    if [ -f ".venv/bin/python" ]; then
        .venv/bin/python scripts/generate_bbob_highdim_proximity_vs_ei_tasks.py
    else
        python3 scripts/generate_bbob_highdim_proximity_vs_ei_tasks.py
    fi
else
...
mkdir -p results/sweep_bbob_highdim_proximity_vs_ei/logs
...
for (( start=START_TASK; start<=END_TASK; start+=CHUNK_SIZE )); do
    ...
    JOB_ID=$(sbatch --parsable --array=${start}-${end}%25 scripts/submit_bbob_highdim_proximity_vs_ei_array.sbatch)
