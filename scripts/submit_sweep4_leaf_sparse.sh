#!/bin/bash
# Submit Sweep 4 (Leaf Sparse Hypercube Gap) as 1 single SLURM Array Job
set -e
...
echo "Submitting Sweep 4 (${TOTAL_TASKS} tasks) as a single SLURM Job Array..."
...
JOB_ID=$(sbatch --parsable ${DEP_FLAG} --job-name=sweep_4_leaf \
...
echo "--------------------------------------------------------"
echo "Sweep 4 successfully scheduled -> SLURM Job ID: ${JOB_ID}"
