#!/usr/bin/env python3
"""Task Generator for CARP-S BBsubset Held-Out Test Evaluation Suite (Proximity LCB vs. SMAC3 EI).
...
def generate_bbsubset_test_proximity_vs_ei_tasks(
    output_file: str | None = "results/sweep_bbsubset_test_proximity_vs_ei/tasks.txt",
    runs_dir: str = "results/sweep_bbsubset_test_proximity_vs_ei",
    baserundir: str = "runs/sweep_bbsubset_test_proximity_vs_ei",
    ...
    """Generates all 1,200 Hydra task command lines for the BBsubset test set evaluation vs SMAC3 EI."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate 1,200 CARP-S BBsubset test suite tasks comparing Proximity LCB vs SMAC3 EI."
    )
    parser.add_argument(
        "--output-file",
        type=str,
        default="results/sweep_bbsubset_test_proximity_vs_ei/tasks.txt",
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.1678,
        help="Exploration floor epsilon for Proximity LCB (default: 0.1678).",
    )
    parser.add_argument(
        "--decay-lambda",
        type=float,
