#!/bin/bash
# Submit Sweep 3 (Fractional Sparse Hypercube Gap) in dependent 250-task chunks (MaxArraySize=300 limit)
set -e
...
echo "Submitting ${TOTAL_TASKS} tasks for Sweep 3 in chunks of ${CHUNK_SIZE} (MaxArraySize compliant)..."
...
for (( start=1; start<=TOTAL_TASKS; start+=CHUNK_SIZE )); do
    ...
    if [ -n "$DEP_FLAG" ]; then
        JOB_ID=$(sbatch --parsable ${DEP_FLAG} --job-name=sweep_3_fract --output="${SWEEP_DIR}/logs/array_%A_%a.log" --error="${SWEEP_DIR}/logs/array_%A_%a.err" --array=${start}-${end}%15 scripts/submit_synthetic_array.sbatch "$SWEEP_DIR")
