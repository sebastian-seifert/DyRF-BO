#!/bin/bash
# Submit Sweep 2 (Linear Sparse Hypercube Gap) as 1 single SLURM Array Job
set -e
...
echo "Submitting Sweep 2 (${TOTAL_TASKS} tasks) as a single SLURM Job Array..."
...
JOB_ID=$(sbatch --parsable ${DEP_FLAG} --job-name=sweep_2_linear \
...
echo "--------------------------------------------------------"
echo "Sweep 2 successfully scheduled -> SLURM Job ID: ${JOB_ID}"
