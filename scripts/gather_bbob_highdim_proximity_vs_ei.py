#!/usr/bin/env python3
"""Data Gathering Script for BBOB High-D (D=16) and Extreme (D=32) Proximity LCB vs SMAC3 EI.

Consolidates raw trial logs in `runs/sweep_bbob_highdim_proximity_vs_ei`
into structured parquet and CSV tables in `results/sweep_bbob_highdim_proximity_vs_ei`.
"""
...
def gather_bbob_highdim_proximity_vs_ei_logs(
    runs_dir: str = "runs/sweep_bbob_highdim_proximity_vs_ei",
    output_dir: str = "results/sweep_bbob_highdim_proximity_vs_ei",
) -> pd.DataFrame:
        ...
        if opt_id == "unknown" or task_id == "unknown" or task_id.startswith("${"):
            for p in run_dir.parts:
                if "ProximityLCB" in p:
                    opt_id = "SMAC20_ProximityLCB"
                elif "HPOFacade_ei" in p or "SMAC3_HPOFacade" in p:
                    opt_id = "SMAC3_HPOFacade_ei"
                if "cfg_16_" in p or "cfg_32_" in p or "/16/" in p or "/32/" in p:
                    task_id = p
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Gather BBOB High-D Proximity vs EI logs.")
    parser.add_argument(
        "--runs-dir",
        type=str,
...
def main() -> None:
    args = parse_args()
    gather_bbob_highdim_proximity_vs_ei_logs(
