...
def parse_aleatoric_masterplan_results(
    ...
    """Parses result JSON files, computes seed averages, groups by metadata, and outputs full tables & Markdown reports."""
    ...
    if baseline in df["approach"].unique():
        baseline_df = df[df["approach"] == baseline].set_index(["func_name", "noise_name", "rf_config", "seed"])
        for app in df["approach"].unique():
            if app == baseline:
                continue
            app_df = df[df["approach"] == app].set_index(["func_name", "noise_name", "rf_config", "seed"])
    ...
    with open(report_file, "w") as mf:
        ...
        mf.write(f"**Total Records Evaluated**: {len(df)} runs across {df['func_name'].nunique()} benchmark target functions, ")
        mf.write(f"{df['noise_name'].nunique()} noise regimes, {df['rf_config'].nunique()} RF hyperparameter configurations, and {df['seed'].nunique()} seeds.\n\n")
