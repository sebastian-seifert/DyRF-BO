#!/bin/bash
# Submit Sweep 5 (Manifold OOD) as 1 single SLURM Array Job
set -e
...
echo "Submitting Sweep 5 (${TOTAL_TASKS} tasks) as a single SLURM Job Array..."
...
JOB_ID=$(sbatch --parsable ${DEP_FLAG} --job-name=sweep_5_manifold \
...
echo "--------------------------------------------------------"
echo "Sweep 5 successfully scheduled -> SLURM Job ID: ${JOB_ID}"
