#!/bin/bash
# ==============================================================================
# Master Slurm Submission Wrapper for Extrapolation UQ Sweep on LUIS Cluster
# ------------------------------------------------------------------------------
...
#
# Because this sweep consists of 1,920 tasks, direct submission of a single
# array (--array=1-1920) is rejected by Slurm with:
#   "sbatch: error: Invalid job array specification"
#
# To comply with LUIS scheduler limits, tasks are dispatched in chunks
# of 200 tasks (array size <= 300, index < 1,000,000) via submit_extrapolation_sweep_all.sh.
#
# USAGE (Run on login node):
#   bash scripts/submit_extrapolation_sweep_slurm.sh
#   (or: bash scripts/submit_extrapolation_sweep_all.sh)
# ==============================================================================
...
if [ -n "$SLURM_JOB_ID" ]; then
    ...
    echo "  bash scripts/submit_extrapolation_sweep_slurm.sh" >&2
    exit 1
fi

DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
exec bash "${DIR}/submit_extrapolation_sweep_all.sh" "$@"
