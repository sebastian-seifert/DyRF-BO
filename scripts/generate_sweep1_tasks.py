...
def main():
    ...
    output_dir = "results/sweep_1_empty"
    ...
    seeds = list(range(1, 8)) # 7 seeds (1..7)
    rf_configs = ["A", "B", "C"]
    lambdas = [0.5, 1.0, 5.0]
    ks = [10, 20, 50]
    alphas = [1.0, 5.0]
    ...
    for func_name in sorted(funcs.keys()):
        for cfg in rf_configs:
            for seed in seeds:
                # Group all proximity approaches into a single combined execution line per (func, cfg, seed)
                # 1. Baselines
                line_base = (
                ...
                # 2. Combined Proximity Sweep Line
                # Passing lists of parameters to compute all proximity method variations in a single RF fit!
                line_prox_combined = (
                    f"--function {func_name} --rf_config {cfg} --seed {seed} --gap_type empty "
                    f"--ood_type hypercube --topological_decay_lambda 0.5,1.0,5.0 --k_neighbors 10,20,50,auto "
    ...
    metadata = {
        "sweep_name": "sweep_1_empty",
        "description": "Empty Hypercube Gap UQ Evaluation (Combined Proximity Execution) across 44 functions, 7 seeds, 3 RF configs",
