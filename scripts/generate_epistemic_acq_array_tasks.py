...
def generate_array_tasks(output_path: str = "results/epistemic_acq_array_tasks.txt") -> list:
    ...
    # Iterate over all 3 acquisition functions (ei, pi, lcb)
    for acq in acquisitions:
        # 1. DyRF-BO Epistemic runs across all 8 approaches (1,040 tasks per acq)
        for task in tasks:
        ...
        # 2. Standard SMAC3 BO baseline runs (130 tasks per acq)
        for task in tasks:
            for seed in seeds:
                line = (
                    ...
                    f"seed={seed} optimizer_id=SMAC3-HPOFacade_{acq} optimizer_container_id=SMAC3"
                )
                lines.append(line)
