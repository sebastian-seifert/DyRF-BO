...
for (( start=1; start<=TOTAL_TASKS; start+=CHUNK_SIZE )); do
    ...
    if [ -n "$DEP_FLAG" ]; then
        JOB_ID=$(sbatch --parsable ${DEP_FLAG} --job-name=carps_scaled --output="${SWEEP_DIR}/logs/array_%A_%a.log" --error="${SWEEP_DIR}/logs/array_%A_%a.err" --array=${start}-${end}%15 scripts/submit_hpobench_array.sbatch)
